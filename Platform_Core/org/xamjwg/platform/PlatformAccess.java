...
public class PlatformAccess {
  ...
  /**
   * Launches the URLs given. Documents are normally opened in a browser frame,
   * unless they include a <code>document-disposition</code> of
   * <code>auto</code> or <code>window</code>. This method attempts to reuse any
   * existing platform JVMs. The method <code>initPlatform</code> should
   ...
   * redirection to console. Implementations that do not care about security do
   * not need to call this before calling <code>launchLocal</code>. This is
   * implicitly called if using one of the <code>launch</code> methods, however.
   * 
   * @see #launchLocal
   */
  ...
  /**
   * Launches the platform without setting up security or a default look & feel,
   * and without reusing any existing platform JVM. Documents are normally
   * opened in a browser frame, unless they include a
  ...
  /**
   * Adds a <code>ClientletSelector</code> which allows the platform to process
   * additional content types. The platform itself registers one or more
   * selectors internally in order to handle XAMJ and other content types. These
