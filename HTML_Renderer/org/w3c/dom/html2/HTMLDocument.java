...
public interface HTMLDocument extends Document {
  ...
  /**
   * A collection of all the anchor (<code>A</code>) elements in a document with
   * a value for the <code>name</code> attribute. For reasons of backward
   ...
   * information described by the <code>cookies</code> non-terminal of [<a
   ...
   * adheres to the <code>cookie</code> non-terminal of [<a
   * href='http://www.ietf.org/rfc/rfc2965.txt'>IETF RFC 2965</a>]; that is, it
   ...
   * attribute for the new value defaults to the absolute path portion of the
   * URI [<a href='http://www.ietf.org/rfc/rfc2396.txt'>IETF RFC 2396</a>] of
   * the current frame or document. If no max-age attribute is specified, then
   * the max-age attribute for the new value defaults to a user agent defined
   * value. If a cookie with the specified name is already associated with the
   * current frame or document, then the new value as well as the new attributes
   * replace the old value and attributes. If a max-age attribute of 0 is
   * specified for the new value, then any existing cookies of the specified
   * name are removed from the cookie storage. See [<a
   ...
   * similar functionality for both HTML and XML documents were being considered
   * (see [<a href='http://www.w3.org/TR/2002/WD-DOM-Level-3-LS-20020725'>DOM
   * Level 3 Load and Save</a>]).
   */
  public void open();
   ...
   * opened by <code>open()</code>. Note that the function will produce a
   ...
   * 4.01</a>] documents, this method returns the (possibly empty) collection of
   * elements whose <code>name</code> value is given by <code>elementName</code>
   * . In [<a href='http://www.w3.org/TR/2002/REC-xhtml1-20020801'>XHTML
   * 1.0</a>] documents, this methods only return the (possibly empty)
